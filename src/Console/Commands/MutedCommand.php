...
class MutedCommand extends Command
{
    ...
    /** @var string */
    protected $signature = 'plume:muted {--max-results=100} {--format=table}';

    /** @var string */
    protected $description = 'List muted users';
    ...
    public function handle(): int
    {
        ...
        try {
            $users = $client->mutedUsers($userId, maxResults: (int) $this->option('max-results'));
        } catch (\Throwable $e) {
        ...
        if (count($users->data) === 0) {
            $this->info('No muted users found.');
