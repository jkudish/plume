...
class PlumeFollowingTool implements Tool
{
    public static function toolId(): string
    {
        return 'plume:following';
    }
    ...
    {
        return 'Fetch the list of users that a user is following.';
    }
    ...
    public function handle(Request $request): string
    {
        ...
        try {
            $result = X::following($userId, $maxResults);

            if ($result->data === []) {
                return 'No following users found.';
            }
            ...
            return "Error fetching following list: {$e->getMessage()}";
        }
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'user_id' => $schema
                ->string()
                ->description('The user ID to get the following list for (defaults to configured X_USER_ID).'),
            'max_results' => $schema
                ->integer()
